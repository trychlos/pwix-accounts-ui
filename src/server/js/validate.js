...
Accounts.validateNewUser(( user ) => {
    ...
    if( AccountsUI.opts().haveEmailAddress() !== AccountsCore.C.Identifier.NONE ){
        _.merge( schema, {
            emails: { type: Array },
            'emails.$': { type: Object },
        ...
        if( AccountsUI.opts().haveEmailAddress() === AccountsCore.C.Identifier.OPTIONAL ){
            schema.emails.optional = true;
        }
    }
    if( AccountsUI.opts().haveUsername() !== AccountsCore.C.Identifier.NONE ){
        _.merge( schema, {
            username: { type: String }
        });
        if( AccountsUI.opts().haveUsername() === AccountsCore.C.Identifier.OPTIONAL ){
    ...
    if( isValid && AccountsUI.opts().haveEmailAddress() !== AccountsCore.C.Identifier.NONE ){
        user.emails.every(( o ) => {
    ...
    if( isValid && AccountsUI.opts().haveUsername() !== AccountsCore.C.Identifier.NONE ){
