 ...
 */
pwixAccounts._identity = function( id ){
    return Meteor.isClient ? Meteor.callPromise( 'pwixAccounts.byId', id ) : pwixAccounts._byId( id );
};
 ...
 */
pwixAccounts._preferredLabelById = function( id, preferred, result ){
    if( Meteor.isClient ){
        return pwixAccounts._identity( id )
            .then(( user ) => {
                if( user ){
                    return pwixAccounts._preferredLabelByDoc( user, preferred );
                }
                console.error( 'id='+id, 'user not found' );
                return result;
            });
    } else {
        const user = pwixAccounts._identity( id );
        return user ? pwixAccounts._preferredLabelByDoc( user, preferred ) : result;
    }
};
 ...
 * @param {String} preferred an optional preference, either pwixAccounts.C.PreferredLabel.USERNAME or pwixAccounts.C.PreferredLabel.EMAIL_ADDRESS,
 *  defaulting to the configured value.
 * @returns: an object:
 *  - label: the label to preferentially use when referring to the user
 *  - origin: whether it was a pwixAccounts.C.PreferredLabel.USERNAME or a pwixAccounts.C.PreferredLabel.EMAIL_ADDRESS
 */
pwixAccounts._preferredLabelByDoc = function( user, preferred, result ){
    let mypref = preferred;
    if( !mypref || !Object.keys( pwixAccounts.C.PreferredLabel ).includes( mypref )){
        mypref = pwixAccounts.opts().preferredLabel();
    }

    if( mypref === pwixAccounts.C.PreferredLabel.USERNAME && user.username ){
        result = { label: user.username, origin: pwixAccounts.C.PreferredLabel.USERNAME };

    } else if( mypref === pwixAccounts.C.PreferredLabel.EMAIL_ADDRESS && user.emails[0].address ){
        result = { label: user.emails[0].address, origin: pwixAccounts.C.PreferredLabel.EMAIL_ADDRESS };
        ...
        result = { label: words[0], origin: pwixAccounts.C.PreferredLabel.EMAIL_ADDRESS };
    }
 ...
 */
pwixAccounts.preferredLabel = function( arg, preferred=pwixAccounts.C.PreferredLabel.USERNAME ){
    ...
    return pwixAccounts['_preferredLabel'+fn]( arg, preferred, result );
};
